...
class WoWBot(discord.Client):
    def __init__(self):
        ...
        self.twitAPI = TwitterAPI('consumer_key',
                                  'consumer_secret',
                                  'access_token_key',
                                  'access_token_secret')
        ...
        if message.content.strip().startswith(self.prefix):
            ...
            if command in [self.prefix + 'help']:
                ...
                for roles in message.author.roles:
                    if roles.name == "Nathrezim" or roles.name == "Man'ari" or roles.name == "Doomguard":
                ...
                if len(args) > 0 and args[0].lower() in ['+', 'add', '-', 'remove', 'list', 'blacklist']:
                    if args[0] in ['+', 'add']:
                        if len(args) == 3:
                            if len(args[1]) > 150 or len(args[2]) > 500:
                        ...
                        try:
                            ...
                            if len(final) > 1800:
                                ...
                                for x in final_this:
                                    await self.timed_message(message.channel, message, x)
                            else:
                                await self.timed_message(message.channel, message, final)
